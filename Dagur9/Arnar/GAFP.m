...
for k = 1:max_gens
    ...
    rank = zeros(N,1);
    for i = 1:N
        rank(i) = sum(func_eval1 < func_eval1(i) & func_eval2<func_eval2(i));
    end
    fitness = 1./rank;
    fprintf('func_eval1 = %s', mat2str(func_eval1))
    fprintf('func_eval2 = %s', mat2str(func_eval2))

    fprintf('Rank = %s ', mat2str(rank))
    fprintf('Rank = %s ', mat2str(rank))
    fprintf('Fitnesses = %s', mat2str(fitness))
    %Distances
