...
figure;
[Pareto_front_ff, Pareto_set_ff] = MOEA(@ff, dimensions_ff, bounds_ff, N, max_gens, pc, pm, sigma_share, [0 1],[0 1]);
...
function [Pareto_front, Pareto_set] = MOEA(objective_funcs, dimensions, bounds, N, max_gens, pc, pm, sigma_share, plotxlim, plotylim)
    ...
    for gen = 1:max_gens
        ...
        archive = update_archive(archive, Population, Obj_values);
        
        % Selection with Pareto dominance tournament
        mating_pool = selection(Population, Obj_values, sigma_share, N);
...
function archive = update_archive(archive, Population, Obj_values)
    ...
    archive = [archive; current_pareto_set];
    archive_obj_values = evaluate_population(archive, @ff);
    % Remove dominated individuals from archive
    [archive_pareto_front, archive_pareto_set] = update_pareto_front(archive, archive_obj_values);
    archive = archive_pareto_set;
end

function mating_pool = selection(Population, Obj_values, sigma_share, N)
    mating_pool = zeros(size(Population));
    for i = 1:N
        parents = tournament_selection(Population, Obj_values);
        mating_pool(i, :) = parents(1, :);
    end
end

function parents = tournament_selection(Population, Obj_values)
    % Select two candidates
    candidate_indices = randi(size(Population, 1), 1, 2);
    candidate1 = candidate_indices(1);
    candidate2 = candidate_indices(2);
