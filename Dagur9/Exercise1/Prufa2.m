...
function [Pareto_front, Pareto_set, allPareto_sets] = MOEA(func1, func2, N, range, max_gens, dimensions)
    ...
    for k = 1:max_gens
        ...
        [Population] = reproduce(Population, func_eval1, func_eval2, dimensions, range, mating_pool);
...
function [new_population] = reproduce(Population, func_eval1, func_eval2, dimensions, range, mating_pool)
    ...
    for i = 1:size(Population, 1)
        if ismember(i,mating_pool)
            if rand <= pc
                % Perform crossover
                parent1 = Population(mating_pool(1), :);
                parent2 = Population(mating_pool(2), :);
