function PackandOpt(N,W,iterations)
    ...
    heights = zeros(1,length(iteration));
    for k = 1:length(iteration)
        [positions_optimized, totalHeight_optimized, newRectangles] = optimizedPacking(rectangles, W, iteration(k), false);
        heights(k) = totalHeight_optimized;
    end
    disp(iteration)
    disp(heights)
    
    semilogx(iteration,heights,'ro-')
    grid on
